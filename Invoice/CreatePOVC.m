...
-(id)initWithPO:(PurchaseOrderOBJ*)sender delegate:(id<PurchaseOrderCreatorDelegate>)del {
	...
	if (self) {
		delegate = del;
		thePurchaseOrder = [[PurchaseOrderOBJ alloc] initWithPO:sender];
	}
...
-(void)viewDidLoad {
  ...
  isNewInvoice = NO;
	if (!thePurchaseOrder) {
  [topBarView setText:@"New P.O."];
  
  thePurchaseOrder = [[PurchaseOrderOBJ alloc] init];
      isNewInvoice = YES;
  } else {
      [topBarView setText:@"Edit P.O."];
  }
  
  [thePurchaseOrder addProjectFieldsToDetails];
  
  if(isNewInvoice) {
    NSString *pref = [PurchaseDefaults purchasePref];
    if ([[thePurchaseOrder number] isEqual:[NSString stringWithFormat:@"%@00001", pref]])
    {
      numberOfDocuments = (int)[CustomDefaults customIntegerForKey:kNumberOfPurchaseOrdersKeyForNSUserDefaults];
      [thePurchaseOrder setNumber:numberOfDocuments];
      [thePurchaseOrder setStringNumber:[thePurchaseOrder number]];
      numberOfDocuments++;
    }
  }
  ...
  openedPO = [[PurchaseOrderOBJ alloc] initWithPO:thePurchaseOrder];
}
...
- (BOOL)isFilledIn {
    ...
    if ([[thePurchaseOrder client] contentsDictionary].count == 0) {
        message = @"Please select a client.";
    }
    
    if ([thePurchaseOrder products].count == 0 && [message isEqual:@""]) {
...
-(void)cancel:(UIButton*)sender {
  if(![thePurchaseOrder isEqualToObject:openedPO]) {
...
-(void)done:(UIButton*)sender {
  [activeTextField resignFirstResponder];
    ...
    if([self isFilledIn]) {
        if ([delegate respondsToSelector:@selector(creatorViewController:createdPO:)]) {
            [delegate creatorViewController:self createdPO:thePurchaseOrder];
        }
...
- (NSString *)numberOfDocumentsKey {
  return kNumberOfPurchaseOrdersKeyForNSUserDefaults;
}
  ...
  SectionTag convertedSection = [self sectionTagInTable:tableView section:section];
    ...
    case SectionClient: {
		...
		case SectionProductsAndServices: {
			return [[thePurchaseOrder products] count] + 1;
...
-(BOOL)tableView:(UITableView*)tableView canEditRowAtIndexPath:(NSIndexPath*)indexPath {
  ...
  if(currentEditingSection >= 0) {
      ...
      if(convertedSection == SectionDetails) {
          ...
          return indexPath.row < [[thePurchaseOrder products] count];
      }
  }
     ...
     isAllowedRow) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        if(currentEditingSection == SectionClient) {
          [thePurchaseOrder moveClientFromIndex:sourceIndexPath.row toIndex:destinationIndexPath.row];
          [self saveClientSettings];
        } else if(currentEditingSection == SectionDetails) {
            [thePurchaseOrder moveDetailFromIndex:sourceIndexPath.row toIndex:destinationIndexPath.row];
            [self saveDetailsSettings];
        } else if(currentEditingSection == SectionProductsAndServices) {
            [thePurchaseOrder moveProductFromIndex:sourceIndexPath.row toIndex:destinationIndexPath.row];
        } else if (currentEditingSection == SectionFigure) {
            [thePurchaseOrder moveFigureFromIndex:sourceIndexPath.row toIndex:destinationIndexPath.row];
    ...
    NSInteger firstRow = currentEditingSection == SectionClient?1:0;
      return [NSIndexPath indexPathForRow:firstRow inSection:currentEditingSection];
             ...
             (convertedproposedSection == SectionProductsAndServices &&
              proposedDestinationIndexPath.row == [[thePurchaseOrder products] count]) ||
              ...
              proposedDestinationIndexPath.row == 0)) {
               ...
               if(currentEditingSection == SectionClient) {
                 newIndex = 1;
                   ...
                   newIndex = [[thePurchaseOrder products] count];
               } else if (currentEditingSection == SectionFigure) {
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if(convertedSection == SectionProductsAndServices) {
      UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"Delete"  handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
          NSMutableArray * temp = [[NSMutableArray alloc] initWithArray:[thePurchaseOrder products]];
          [temp removeObjectAtIndex:indexPath.row];
          [thePurchaseOrder setProducts:temp];
          [tableView reloadData];
          
          [sync_manager updateCloud:[thePurchaseOrder contentsDictionary] andPurposeForDelete:0];
      }];
      return @[deleteAction];
  } else if (convertedSection == SectionDetails) {
      DetailsType type = [self detailTypeAtIndex:indexPath.row];
      if(type >= DetailCustom1 && type <= DetailCustom5) {
          UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"Delete"  handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
              [thePurchaseOrder removeCustomDetailFieldAtType:[self detailTypeAtIndex:indexPath.row]];
...
-(BaseTableCell*)cellInSection:(int)section atRow:(int)row {
    ...
    case SectionClient: {
      ...
      BaseTableCell *cell = [self detaildeCellAtRow:(int)cellType];
...
-(BaseTableCell*)titleCellAtRow:(int)row {
  ...
  switch (row) {
      case 0: {
          ...
          [(CellWithEditField*)theCell loadTitle:[thePurchaseOrder title] tag:TITLE_CELL_TAG textFieldDelegate:self cellType:kCellTypeSingle andKeyboardType:UIKeyboardTypeDefault];
...
- (BaseTableCell *)yourCompanyCellAtRow:(int)row {
...
-(BaseTableCell*)detaildeCellAtRow:(int)row {
  BaseTableCell * theCell;
  ...
  switch (row) {
    case DetailProjNumber: {
        ...
        if (!theCell) {
            theCell = [[CellWithText alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellText" width:detailTable.frame.size.width];
        }
        
        NSString * valueSTR = [thePurchaseOrder number];
        
        [(CellWithText*)theCell loadTitle:[thePurchaseOrder detailTitleForType:row] andValue:valueSTR tag:444 textFieldDelegate:self cellType:[self detailsCellTypeForRowType:row] andKeyboardType:UIKeyboardTypeDefault];
    ...
    case DetailCustom5: {
        ...
        if (!theCell) {
            theCell = [[CellWithEditField alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellEditField" width:detailTable.frame.size.width];
        }
    ...
    case DetailProjectName: {
      ...
      [(CellWithPush*)theCell loadTitle:@"Project Name" andValue:[[thePurchaseOrder project] projectName] cellType:[self detailsCellTypeForRowType:row] andSize:20.0];
    ...
    case DetailProjectNo: {
      ...
      [(CellWithPush*)theCell loadTitle:@"Project No." andValue:[[thePurchaseOrder project] projectNumber] cellType:[self detailsCellTypeForRowType:row] andSize:20.0];
      theCell.canEditvalueField = NO;
    ...
    case DetailAddNewLine: {
      ...
      if (!theCell) {
          theCell = [[CellWithPush alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellPushAddNewLine" width:detailTable.frame.size.width];
      }
      [(CellWithPush*)theCell removeValueField];
      theCell.isTitleEditable = NO;
      
      kCellType type = kCellTypeBottom;
      if ([[thePurchaseOrder visibleRowsInDetailsSection] count] == 0)
      ...
      [theCell setUserInteractionEnabled:[thePurchaseOrder numberOfCustomDetailFieldsVisibleOnly:NO] < CUSTOM_FIELDS_MAX_COUNT];
...
- (BaseTableCell *)clientCellAtRow:(int)row {
  ...
  switch (row) {
    case ClientAddClient: {
      ...
      if (!theCell) {
        theCell = [[CellWithPush alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellPushClient" width:clientTable.frame.size.width];
      }
      
      ClientOBJ * theClient = [thePurchaseOrder client];
    ...
    case ClientBilling: {
      ...
      if (!theCell) {
        theCell = [[CellWithPush alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellPushArrow" width:clientTable.frame.size.width];
      }
...
-(BaseTableCell*)productCellAtRow:(int)row {
  ...
  UITableView *productTable = [self tableViewForSection:SectionProductsAndServices];
	...
	if (row < [[thePurchaseOrder products] count]) {
    theCell = [productTable dequeueReusableCellWithIdentifier:@"tableCellText"];
    
    if (!theCell) {
        theCell = [[CellWithText alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellText" width:productTable.frame.size.width];
    }
		...
		NSDictionary * dict = [[thePurchaseOrder products] objectAtIndex:row];
		
		NSString * classString = [dict objectForKey:@"class"];
		
		if (NSClassFromString(classString) == [ProductOBJ class]) {
			ProductOBJ * theProduct = [[ProductOBJ alloc] initWithContentsDictionary:[[thePurchaseOrder products] objectAtIndex:row]];
			...
			ServiceOBJ * theService = [[ServiceOBJ alloc] initWithContentsDictionary:[[thePurchaseOrder products] objectAtIndex:row]];
    ...
    [(CellWithText*)theCell setTitleEditableLayout];
		...
		if (!theCell) {
			theCell = [[CellWithPush alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableCellPush" width:productTable.frame.size.width];
		}
  ...
  UITableView *valuesTable = [self tableViewForSection:SectionFigure];
	...
	switch (row) {
		case FigureSubtotal: {
			...
			[(CellWithText*)theCell loadTitle:[thePurchaseOrder figureTitleForType:row] andValue:[data_manager currencyAdjustedValue:[thePurchaseOrder subtotal]] tag:0 textFieldDelegate:self cellType:[self figureCellTypeForRowType:FigureSubtotal] andKeyboardType:UIKeyboardTypeDefault];
      [theCell setUserInteractionEnabled:YES];
      ...
      [theCell setIsTitleEditable:YES delegate:self tag:FiguresTextfieldTitleOffset + row];
		...
		case FigureDiscount: {
			...
			[(CellWithText*)theCell loadTitle:[thePurchaseOrder figureTitleForType:row]
							 andValue:[data_manager currencyAdjustedValue:[thePurchaseOrder discount]] tag:111 textFieldDelegate:self cellType:[self figureCellTypeForRowType:FigureDiscount] andKeyboardType:UIKeyboardTypeDecimalPad];
      [theCell setUserInteractionEnabled:YES];
      [theCell setIsEditingMode:valuesTable.isEditing];
      ...
      NSString * title = [NSString stringWithFormat:@"%@ (%.2f%c)", [thePurchaseOrder tax1Name], [thePurchaseOrder tax1Percentage], '%'];
      
      NSString * val = [data_manager currencyAdjustedValue:[thePurchaseOrder tax1Value]];
      ...
      NSString * title = [NSString stringWithFormat:@"%@ (%.2f%c)", [thePurchaseOrder tax2Name], [thePurchaseOrder tax2Percentage], '%'];
      
      NSString * val = [data_manager currencyAdjustedValue:[thePurchaseOrder tax2Value]];
      ...
      [(CellWithText*)theCell loadTitle:[thePurchaseOrder figureTitleForType:row] andValue:[data_manager currencyAdjustedValue:[thePurchaseOrder shippingValue]]
      ...
      [theCell setIsTitleEditable:YES delegate:self tag:FiguresTextfieldTitleOffset + row];
		...
		case FigureTotal: {
			...
			[(CellWithText*)theCell loadTitle:[thePurchaseOrder figureTitleForType:row] andValue:[data_manager currencyAdjustedValue:[thePurchaseOrder total]] tag:0 textFieldDelegate:self cellType:[self figureCellTypeForRowType:FigureTotal] andKeyboardType:UIKeyboardTypeDefault];
      [theCell setUserInteractionEnabled:YES];
      [theCell setIsEditingMode:valuesTable.isEditing];
      [theCell setCanEditvalueField:NO];
      [theCell setIsTitleEditable:YES delegate:self tag:FiguresTextfieldTitleOffset + row];
  ...
  UITableView *otherTable = [self tableViewForSection:SectionOptionalInfo];
	...
	switch (row) {
		case 0: {
			...
			[(CellWithPush*)theCell loadTitle:[thePurchaseOrder otherCommentsTitle] andValue:[thePurchaseOrder otherCommentsText] cellType:kCellTypeTop andSize:0.0];
			[theCell setUserInteractionEnabled:YES];
      ...
      NSString *signTitle = [thePurchaseOrder leftSignatureTitle];
      ...
      NSString *signTitle = [thePurchaseOrder rightSignatureTitle];
      ...
      [theCell makeValueBigger:YES];
			[(CellWithPush*)theCell loadTitle:@"Note" andValue:[thePurchaseOrder note] cellType:kCellTypeBottom andSize:0.0];
			[theCell setUserInteractionEnabled:YES];
			break;
		}
...
-(void)selectedTitleCellAtRow:(int)row {
	switch (row) {
		case 0: {
			EditTitleVC * vc = [[EditTitleVC alloc] initWithPO:thePurchaseOrder];
...
-(void)selectedDetaildeCellAtRow:(int)row {
    ...
    switch (row) {
      ...
      case DetailProjectName:
      case DetailProjectNo: {
        SelectProjectVC *vc = [[SelectProjectVC alloc] initWithPO:thePurchaseOrder];
        [self.navigationController pushViewController:vc animated:YES];
...
- (void)showClientScreen {
  ClientsVC * vc = [[ClientsVC alloc] initWithPO:thePurchaseOrder];
  [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)selectedClientCellAtRow:(int)row {
  switch (row) {
    ...
    case ClientBilling: {
      ClientOBJ * theClient = [thePurchaseOrder client];
      theClient.billingKey = kPurchaseBillingAddressTitleKeyForNSUserDefaults;
    ...
    case ClientShipping: {
      ClientOBJ * theClient = [thePurchaseOrder client];
      theClient.shippingKey = kPurchaseShippingAddressTitleKeyForNSUserDefaults;
...
-(void)selectedProductCellAtRow:(int)row {
	AddItemVC * vc = [[AddItemVC alloc] initWithPO:thePurchaseOrder index:row];
	[vc setDelegate:self];
	[self.navigationController pushViewController:vc animated:YES];
}
...
-(void)selectedValuesCellAtRow:(int)row {
	...
	if (![[thePurchaseOrder tax1Name] isEqual:@""]) {
		has_tax1 = YES;
	}
	
	if (![[thePurchaseOrder tax2Name] isEqual:@""]) {
		has_tax2 = YES;
	}
...
-(void)selectedOtherCellAtRow:(int)row {
	switch (row) {
		case 0: {
			//Other Comments
			OtherCommentsVC * vc = [[OtherCommentsVC alloc] initWithPurchaseOrder:thePurchaseOrder];
			[self.navigationController pushViewController:vc animated:YES];
		...
		case 1: {
			//Signature Left
			AddSignatureAndDateVC * vc = [[AddSignatureAndDateVC alloc] initWithDelegate:self andPurchaseOrder:thePurchaseOrder type:kSignatureTypeLeft];
			[self.navigationController pushViewController:vc animated:YES];
		...
		case 2: {
			//Signature Right
			AddSignatureAndDateVC * vc = [[AddSignatureAndDateVC alloc] initWithDelegate:self andPurchaseOrder:thePurchaseOrder type:kSignatureTypeRight];
			[self.navigationController pushViewController:vc animated:YES];
		...
		case 3: {
			//Note
			AddNoteVC * vc = [[AddNoteVC alloc] initWithPO:thePurchaseOrder];
			[self.navigationController pushViewController:vc animated:YES];
...
-(void)viewController:(AddItemVC*)vc addedProduct:(ProductOBJ*)sender atIndex:(NSInteger)index {
	NSMutableArray * array = [[NSMutableArray alloc] initWithArray:thePurchaseOrder.products];
	...
	[thePurchaseOrder setProducts:array];
}
...
-(void)closePicker:(UIButton*)sender{
	for (int i = 0; i < [thePurchaseOrder products].count; i++) {
...
-(void)textFieldDidEndEditing:(UITextField*)textField {
  ...
  if(textField.tag == TITLE_CELL_TAG) {
    if([textField.text length] > 0) {
        [thePurchaseOrder setTitle:textField.text];
    } else {
        textField.text = [thePurchaseOrder title];
    }
  ...
  if(textField.tag == CLIENT_NAME_CELL_TAG) {
    ClientOBJ *client = [thePurchaseOrder client];
    [client setFirstName:textField.text];
    [thePurchaseOrder setClient:[client contentsDictionary]];
  ...
  if(textField.tag >= TextfieldOffsetTag) {
    ...
    sectionTag = SectionFigure;
		...
		switch (textField.tag) {
			case 111: {
				if (value.selected) {
					[thePurchaseOrder setDiscount:[textField.text floatValue]];
				} else {
					float x = [textField.text floatValue];
					float sum = [thePurchaseOrder subtotal];
					float y = (x * sum) / 100;
					
					[thePurchaseOrder setDiscount:y];
			...
			case 222: {
				[thePurchaseOrder setShippingValue:[textField.text floatValue]];
				break;
			}
			...
			case 444: {
				[thePurchaseOrder setStringNumber:textField.text];
				break;
			}
...
-(void)creatorViewController:(AddSignatureAndDateVC*)viewController createdSignature:(UIImage*)signature withFrame:(CGRect)frame title:(NSString*)title andDate:(NSDate*)date {
	switch (viewController.signatureType) {
		case kSignatureTypeLeft: {
			[thePurchaseOrder setLeftSignature:signature];
			[thePurchaseOrder setLeftSignatureDate:date];
			[thePurchaseOrder setLeftSignatureTitle:title];
			[thePurchaseOrder setLeftSignatureFrame:frame];
      
      if([thePurchaseOrder alwaysShowSignatureLeft]) {
        [thePurchaseOrder saveLeftSignature:signature];
        [thePurchaseOrder saveLeftSignatureTitle:title];
        [thePurchaseOrder saveLeftSignatureFrame:frame];
      } else {
        [thePurchaseOrder removeLeftCommonSignatureKey];
        [thePurchaseOrder saveLeftSignatureTitle:@"Signature"];
      }
		...
		case kSignatureTypeRight: {
			[thePurchaseOrder setRightSignature:signature];
			[thePurchaseOrder setRightSignatureDate:date];
			[thePurchaseOrder setRightSignatureTitle:title];
			[thePurchaseOrder setRightSignatureFrame:frame];
      
      if([thePurchaseOrder alwaysShowSignatureRight]) {
        [thePurchaseOrder saveRightSignature:signature];
        [thePurchaseOrder saveRightSignatureTitle:title];
        [thePurchaseOrder saveRightSignatureFrame:frame];
      } else {
        [thePurchaseOrder removeRightCommonSignatureKey];
        [thePurchaseOrder saveRightSignatureTitle:@"Signature"];
      }
