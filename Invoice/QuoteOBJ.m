...
-(id)init
{
	...
	if (self)
	{
		...
		[self setID:[data_manager createQuoteID]];
		
		[contents setObject:kQuoTitle forKey:@"title"];
		[self setName:@""];
		[self setClient:[[[ClientOBJ alloc] init] contentsDictionary]];
    [self setClientBillingTitleforKey:kQuoteBillingAddressTitleKeyForNSUserDefaults
		...
		[self setNote:QUOTE_FIRST_PAGE_NOTE];
		[self setBigNote:QUOTE_SECOND_PAGE_NOTE];
    
		[self setOtherCommentsTitle:QUOTE_OTHER_COMMENTS_TITLE];
		[self setOtherCommentsText:QUOTE_OTHER_COMMENTS_TEXT];
    
    [self setRightSignatureTitle:QUOTE_RIGHT_SIGNATURE_TITLE];
    [self setRightSignatureDate:[date_formatter dateFromString:[date_formatter stringFromDate:[NSDate date]]]];
		[self setRightSignature:[UIImage imageWithContentsOfFile:[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",QUOTE_RIGHT_SIGNATURE_PATH]]];
		[self setRightSignatureFrame:QUOTE_RIGHT_FRAME];
    
    [self setLeftSignatureTitle:QUOTE_LEFT_SIGNATURE_TITLE];
    [self setLeftSignatureDate:[date_formatter dateFromString:[date_formatter stringFromDate:[NSDate date]]]];
		[self setLeftSignature:[UIImage imageWithContentsOfFile:[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",QUOTE_LEFT_SIGNATURE_PATH]]];
		[self setLeftSignatureFrame:QUOTE_LEFT_FRAME];
...
-(id)initWithQuote:(QuoteOBJ*)sender
{
    ...
    [date_formatter setDateFormat:[DELEGATE userSelectedDateFormat]];
		...
		if (sender && [sender isKindOfClass:[QuoteOBJ class]])
		{
...
- (NSString *)alwaysShowLeftSignatureKey {
  return kQuoteAlwaysShowOptionalInfoSignatureLeft;
}

- (NSString *)alwaysShowRightSignatureKey {
  return kQuoteAlwaysShowOptionalInfoSignatureRight;
}

- (NSString *)alwaysShowNoteKey {
  return kQuoteAlwaysShowOptionalInfoNote;
}

- (NSString *)alwaysShowOptionalInfoKey {
  return kQuoteAlwaysShowOptionalInfoOtherComments;
}

#pragma mark - Figure settings methods

- (NSArray *)localFigureSettings {
    return [data_manager quoteFigureSettings];
}

- (void)saveFigureSettings {
    [data_manager setQuoteFiguresSettings:[self figuresSettings]];
}
...
- (NSArray *)localDetailsSettings {
    ...
    if([self areFiguresSettingsInDetailsFile2Quote]) {
      [data_manager setQuoteDetailsSettings:[self newDetailsSettings]];
    }
  
    return [data_manager quoteDetailsSettings];
}

- (void)saveDetailsSettings {
    [data_manager setQuoteDetailsSettings:[self detailsSettings]];
}

- (void)setAndSaveDetailsSettings:(NSArray *)details {
  [data_manager setQuoteDetailsSettings:details];
}

#pragma mark - AddItem Details settings methods

- (NSArray *)localAddItemDetailsSettings {
  return [data_manager quoteAddItemDetailsSettings];
}

- (void)saveAddItemDetailsSettings {
    [data_manager setQuoteAddItemDetailsSettings:[self addItemDetailsSettings]];
}

- (void)setAndSaveAddItemDetailsSettings:(NSArray *)settings {
  [data_manager setQuoteAddItemDetailsSettings:settings];
}

#pragma mark - Profile settings methods

- (NSArray *)localProfileSettings {
  return [data_manager quoteProfileSettings];
}

- (void)saveProfileSettings {
  [data_manager setQuoteProfileSettings:[self profileSettings]];
}

#pragma mark - Client settings methods
- (NSArray *)localClientSettings {
  return [data_manager quoteDetailsClientSettings];
}

- (void)saveClientSettings {
  [data_manager setQuoteClientDetailsSettings:[self clientSettings]];
}
...
-(NSString*)title
{
	...
	{
		[data_manager logGetterErrorFromClass:[self class] forProperty:@"title" containedValue:[contents objectForKey:@"title"] withDefautReturnValue:kQuoTitle];
		return kQuoTitle;
	}
}
...
-(NSString*)otherCommentsTitle
{
	...
	else
	{
		...
		return QUOTE_OTHER_COMMENTS_TITLE;
	}
}
...
-(NSString*)otherCommentsText
{
	...
	else
	{
		...
		return QUOTE_OTHER_COMMENTS_TEXT;
	}
}
...
-(NSString*)rightSignatureTitle
{
	...
	else
	{
		...
		return QUOTE_RIGHT_SIGNATURE_TITLE;
	}
}
...
-(UIImage*)rightSignature
{
	...
	else
	{
		...
		return [UIImage imageWithContentsOfFile:[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",QUOTE_RIGHT_SIGNATURE_PATH]];
	}
}
...
-(NSString*)leftSignatureTitle
{
	...
	else
	{
		...
		return QUOTE_LEFT_SIGNATURE_TITLE;
	}
}
...
-(UIImage*)leftSignature
{
	...
	else
	{
		...
		return [UIImage imageWithContentsOfFile:[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",QUOTE_LEFT_SIGNATURE_PATH]];
	}
}
...
-(void)setTitle:(NSString*)sender
{
	if (sender && [sender isKindOfClass:[NSString class]])
	{
		[CustomDefaults setCustomObjects:sender forKey:kQuoteTitleKeyForNSUserDefaults];
		...
		NSMutableDictionary *quoteDict = [[NSMutableDictionary alloc] initWithDictionary:[languageDict objectForKey:@"quote"]];
		[quoteDict setObject:sender forKey:@"Quote"];
	...
	{
		[data_manager logSetterErrorFromClass:[self class] forProperty:@"title" sentValue:sender withDefaultSetValue:kQuoTitle];
		[contents setObject:kQuoTitle forKey:@"title"];
	}
}
...
- (void)saveNote:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]]) {
    [CustomDefaults setCustomString:sender forKey:QUOTE_FIRST_PAGE_NOTE_KEY];
  }
}
...
- (void)saveBigNote:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]]) {
    if (![[sender uppercaseString] isEqual:@"BIG NOTE"]) {
      [CustomDefaults setCustomString:sender forKey:QUOTE_SECOND_PAGE_NOTE_KEY];
    }
  }
}

-(void)setNumber:(NSInteger)sender
{
	[contents setObject:[NSNumber numberWithInteger:sender] forKey:@"number"];
}
...
- (void)saveOtherCommentsTitle:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]])
  {
    [CustomDefaults setCustomString:sender forKey:QUOTE_OTHER_COMMENTS_TITLE_KEY];
  }
}
...
- (void)saveOtherCommentsText:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]])
  {
    [CustomDefaults setCustomString:sender forKey:QUOTE_OTHER_COMMENTS_TEXT_KEY];
  }
}
...
- (void)saveRightSignatureTitle:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]]) {
    [CustomDefaults setCustomString:sender forKey:QUOTE_RIGHT_SIGNATURE_TITLE_KEY];
  }
}
...
- (void)setRightSignature:(UIImage *)sender andSave:(BOOL)save {
  ...
  if(save) {
    [CustomDefaults setCustomString:MD5 forKey:QUOTE_RIGHT_SIGNATURE_PATH_KEY];
  } else {
...
-(void)removeRightSignatureImageAndSave:(BOOL)save {
  ...
  if((![signFilePath isEqualToString:QUOTE_RIGHT_SIGNATURE_PATH] && !save) || save) {
    NSString *imagePath = save?QUOTE_RIGHT_SIGNATURE_PATH:signFilePath;
  ...
  if(save) {
    [CustomDefaults setCustomString:@"" forKey:QUOTE_RIGHT_SIGNATURE_PATH_KEY];
  } else {
...
- (void)saveRightSignatureFrame:(CGRect)sender {
  [CustomDefaults setCustomObjects:[NSKeyedArchiver archivedDataWithRootObject:[NSValue valueWithCGRect:sender]] forKey:QUOTE_RIGHT_FRAME_KEY];
}
...
- (void)saveLeftSignatureTitle:(NSString *)sender {
  if (sender && [sender isKindOfClass:[NSString class]])
  {
    [CustomDefaults setCustomString:sender forKey:QUOTE_LEFT_SIGNATURE_TITLE_KEY];
  }
}
...
- (void)setLeftSignature:(UIImage *)sender andSave:(BOOL)save {
  ...
  if(save) {
    [CustomDefaults setCustomString:MD5 forKey:QUOTE_LEFT_SIGNATURE_PATH_KEY];
  } else {
...
-(void)removeLeftSignatureImageAndSave:(BOOL)save {
  ...
  if((![signFilePath isEqualToString:QUOTE_LEFT_SIGNATURE_PATH] && !save) || save) {
    NSString *imagePath = save?QUOTE_LEFT_SIGNATURE_PATH:signFilePath;
    NSString *filePath = [NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",imagePath];
  ...
  if(save) {
    [CustomDefaults setCustomString:@"" forKey:QUOTE_LEFT_SIGNATURE_PATH_KEY];
  } else {
...
{
	NSString *filePath = [NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",[contents objectForKey:@"leftSignatureFilePath"]];
...
- (void)saveLeftSignatureFrame:(CGRect)sender {
  [CustomDefaults setCustomObjects:[NSKeyedArchiver archivedDataWithRootObject:[NSValue valueWithCGRect:sender]] forKey:QUOTE_LEFT_FRAME_KEY];
}
