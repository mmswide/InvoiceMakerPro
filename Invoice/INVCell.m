...
-(void)loadInvoice:(InvoiceOBJ*)sender withCellType:(kInvoiceCellType)type
{
	...
	[priceLabel setText:[data_manager currencyAdjustedValue:[sender balanceDuePaid]]];
	[date_formatter setDateFormat:[DELEGATE userSelectedDateFormat]];
	[dueLabel setText:[data_manager dueStringForDate:[NSDate date] andDate:[date_formatter dateFromString:[sender dueDate]]]];
	[statusLabel setText:[InvoiceOBJ statusTextFor:[sender status]]];
	
	if ([sender status] == kInvoiceStatusPaid)
	{
		[dueLabel setText:@""];
	}
}
