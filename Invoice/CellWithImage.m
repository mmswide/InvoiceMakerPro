...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier
{
	...
	if (self)
	{
		...
		cellImage = [[UIImageView alloc] initWithFrame:CGRectMake(dvc_width - 20 - 60, 2, 38, 38)];
		[cellImage setContentMode:UIViewContentModeScaleAspectFit];
		cellImage.layer.borderWidth = 0.5;
		cellImage.layer.borderColor = [UIColor lightGrayColor].CGColor;
		cellImage.userInteractionEnabled = NO;
		[self addSubview:cellImage];
		
		UIImageView *plusImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
		[plusImage setImage:[UIImage imageNamed:@"plus.png"]];
		[self addSubview:plusImage];
		
		[plusImage setCenter:CGPointMake(dvc_width - 30, 21)];		
	}
...
-(void)loadTitle:(NSString*)title andValue:(NSData*)imageData tag:(int)tag cellType:(kCellType)type
{
	...
	{
		cellImage.image = nil;
		cellImage.layer.borderWidth = 0.0;
	}
